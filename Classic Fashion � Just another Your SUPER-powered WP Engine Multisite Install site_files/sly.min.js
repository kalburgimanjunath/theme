...
;(function ($, w, undefined) {
	...
	function Sly(frame, o, callbackMap) {
		...
		var $activeElement = $('.frame ul li').eq(rel.activeItem+1);
		var $activeElemIndex = 0;
		var callbacks = {};
		var last = {};
		var animation = {};
		...
		/**
		 * Try to lazy load images
		 *
		 * @return {Void}
		 */
		function lazyLoadImages(){
			if($('div.additional_items li').length){ //if there are images inside div.additional_items 
				var hz = $('div.additional_items li:first-child'); // get the first image
				var original_image_src = $(hz).find('img').data('original'); // find the original source of the first image
				var img_width = $(hz).find('img').attr('width');
				var img_height = $(hz).find('img').attr('height');
				var container_height = $('.entry-header div.frame').height();

				/*calculate the width and height for Firefox mainly*/
				//if($.browser.mozilla){
					if(img_height > container_height){
						var resized_width = container_height*img_width/img_height;
						$(hz).css('width',parseInt(resized_width));
						$(hz).css('height',parseInt(container_height));
						
					}else{
						$(hz).css('width',parseInt(img_width));
						$(hz).css('height',parseInt(img_height));
					}	
				//}
				

				$(hz).find('img').attr('src',original_image_src); // replace teh dummy source with the original image source 
				self.add(hz); // add this image
		...
		function syncScrollbar() {
			...
			$activeElement = $('.frame ul li.active');
			//lazyLoadImages();
			$activeElement.on(clickEvent, buttonsHandler);
		}
		...
		function buttonsHandler(event) {
			...
			stopDefault(event);
			switch (this) {
				case $activeElement[0]:
					$activeElement = $('.frame ul li').eq(rel.activeItem);
					setTimeout(function(){
						self.next();
					},100);
					break;
