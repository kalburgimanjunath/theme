...
jQuery(document).ready(function () {
	if ((jQuery("#supersized").length > 0) && supersized_slides) {
		if (typeof supersized_autoplay == 'undefined') supersized_autoplay = 1;
		if (typeof supersized_slide_interval == 'undefined') supersized_slide_interval = 5000;
		...
		theme = {
			beforeAnimation : function(state) {
				$title = api.getField('article_title');
				$text = api.getField('article_text');
				if (!$title) {
					jQuery('#supersized-info').hide();
				} else {
					jQuery('#supersized-info').show();
					$href = api.getField('article_url');
					if ($href) {
						$a = jQuery('<a></a>').html($title).attr('href',$href);
					} else {
						$a = $title;
					}
					jQuery('#supersized-info h2').html($a);
					if ($text) {
						jQuery('#supersized-info p').removeClass('no-padding');
					} else {
						jQuery('#supersized-info p').addClass('no-padding');
					}
					jQuery('#supersized-info p').html($text);
				}
			}
		}
	}
